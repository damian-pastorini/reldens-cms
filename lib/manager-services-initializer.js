...
const { DriversMap, PrismaClientLoader, PrismaSchemaGenerator } = require('@reldens/storage');
...
class ManagerServicesInitializer
{
    ...
    async initializeDataServer()
    {
        ...
        if('prisma' === this.manager.config.database.driver){
            dbConfig.client = new PrismaSchemaGenerator({client: dbConfig.client}).getDatasourceProvider();
        ...
        let modulesProp = StorageDriversResolver.modulesProp(this.manager.config.database.driver);
        if('prismaModules' !== modulesProp){
            if(!this.manager[modulesProp]){
                this.manager[modulesProp] = StorageDriversResolver.loadModules(
                    this.manager.config.database.driver,
                    this.manager.projectRoot,
                    this.manager.config.database.client
                );
            }
            if(!this.manager[modulesProp]){
                Logger.critical('Failed to load storage driver modules: '+this.manager.config.database.driver);
                return false;
            }
            dbConfig[modulesProp] = this.manager[modulesProp];
        }
    ...
    async initializeAdminManager()
    {
        ...
        if('db-users' === this.manager.authenticationMethod && !authenticationCallback){
            authenticationCallback = async (email, password, roleId) => {
                ...
                let passwordResult = await Encryptor.validatePassword(password, user.password) ? user : false;
                if(!passwordResult){
