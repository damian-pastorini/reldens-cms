...
const { StorageDriversResolver } = require('./storage-drivers-resolver');
...
class Installer
{
    ...
    async executeForEveryRequest(req, res, next)
    {
        ...
        if('' === urlPath || '/' === urlPath){
            ...
            contentParams.storageDrivers = this.storageDriversOptions(contentParams['db-storage-driver']);
    ...
    storageDriversOptions(selectedDriver)
    {
        return StorageDriversResolver.available(this.projectRoot, this.prismaAdapter).map(driver => {
            return {key: driver.key, label: driver.label, selected: selectedDriver === driver.key ? ' selected' : ''};
        });
    }
    ...
    getErrorMessage(errorCode)
    {
        let errorMessages = {
            ...
            'driver-packages-missing': 'Storage driver packages not found in the project.',
    ...
    async executeInstallProcess(req, res)
    {
        ...
        let dbConfig = {
            client: sc.get(templateVariables, 'db-client', 'mysql2'),
            config: {
        ...
        if(!this.appendDriverModules(dbConfig, selectedDriver)){
    ...
    appendDriverModules(dbConfig, selectedDriver)
    {
        if('prisma' === selectedDriver){
            return true;
        }
        let modulesProp = StorageDriversResolver.modulesProp(selectedDriver);
        dbConfig[modulesProp] = StorageDriversResolver.loadModules(selectedDriver, this.projectRoot, dbConfig.client);
        return Boolean(dbConfig[modulesProp]);
    }
    ...
    mapVariablesForTemplate(configVariables)
    {
        return {
            ...
            dbUrlClient: MySQLInstaller.normalizePrismaProvider(configVariables.database.client),
    ...
    mapVariablesForConfig(templateVariables)
    {
        return {
            ...
            database: {
                ...
                driver: sc.get(templateVariables, 'db-storage-driver', 'knex')
            }
        };
    }
    ...
    fetchDefaults()
    {
        return {
            ...
            'db-storage-driver': process.env.RELDENS_STORAGE_DRIVER || 'knex',
            'db-client': process.env.RELDENS_DB_CLIENT || 'mysql2',
