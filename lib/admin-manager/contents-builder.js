...
class ContentsBuilder
{
    ...
    async buildEntitiesContents()
    {
        ...
        for(let driverResource of this.resources()){
            ...
            let editFields = this.buildEditFields(editProperties, driverResource.options.properties, entityId);
    ...
    buildEditFields(editProperties, properties, entityId)
    {
        let editFields = [];
        for(let propertyKey of editProperties){
            let isRequired = sc.get(properties[propertyKey], 'isRequired', false);
            let isUnique = sc.get(properties[propertyKey], 'isUnique', false);
            editFields.push({
                name: this.fetchTranslation(propertyKey, entityId),
                value: '{{&'+propertyKey+'}}',
                isRequired,
                isUnique,
                hasBothMarkers: isRequired && isUnique
            });
        }
        return editFields;
    }
