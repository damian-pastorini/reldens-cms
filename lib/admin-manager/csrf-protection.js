...
class CsrfProtection
{
    ...
    isValidRequest(req)
    {
        ...
        let requestTokenBuffer = Buffer.from(requestToken);
        let sessionTokenBuffer = Buffer.from(String(sessionToken));
        if(requestTokenBuffer.length !== sessionTokenBuffer.length){
            return false;
        }
        return crypto.timingSafeEqual(requestTokenBuffer, sessionTokenBuffer);
    }
