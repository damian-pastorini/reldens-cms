...
class RouterContents
{
    ...
    async generateEditRouteContent(req, driverResource, entityPath)
    {
        ...
        for(let propertyKey of Object.keys(driverResource.options.properties)){
            ...
            let isKeptOnEdit = property.isUpload || -1 !== this.passwordFieldNames.indexOf(propertyKey);
            ...
            let templateData = {
                ...
                required: (!isKeptOnEdit || !loadedEntity) && property.isRequired ? ' required="required"' : '',
    ...
    async processSaveEntity(req, res, driverResource, entityPath)
    {
        ...
        try {
            ...
            if(!saveResult){
                Logger.error(
                    'Save result error.',
                    {entity: driverResource.entityKey, id, fields: Object.keys(entityDataPatch)}
                );
            ...
            Logger.error(
                'Save entity error.',
                {entity: driverResource.entityKey, id, code: error?.code, sqlMessage: error?.sqlMessage}
            );
    ...
    preparePatchData(driverResource, idProperty, req, resourceProperties, id, loadedEntity)
    {
        ...
        for(let i of Object.keys(resourceProperties)){
            ...
            if(-1 !== this.passwordFieldNames.indexOf(i) && !propertyUpdateValue){
                if(id || !property.isRequired){
                    continue;
                }
            }
