...
class Frontend
{
    ...
    {
        if(sc.get(content, 'template', '')){
            let templatePath = this.findTemplatePath(content.template, domain);
            if(templatePath){
                let templateContent = await this.renderContentWithTemplate(templatePath, data, domain);
                if(templateContent){
