...
class Frontend
{
    ...
    async handleRequest(req, res)
    {
        try {
            ...
            if(route){
                ...
                return await this.renderWithCacheHandler(
                    async () => await this.generateRouteContent(route, domain, req),
                    async () => await this.renderNotFound(domain, res, req),
            ...
            if(entityResult){
                return await this.renderWithCacheHandler(
                    async () => await this.renderWithTemplateContent(
                        entityResult.entity,
                        {},
                        domain,
                        req,
            ...
            if(templatePath){
                return await this.renderWithCacheHandler(
                    async () => await this.generateTemplateContent(templatePath, domain, req),
                    async () => res.status(500).send('Template error: '+templatePath),
                    async (content) => res.send(await this.renderWithTemplateContent({content}, {}, domain, req, null)),
                    domain,
                    res,
                    originalPath,
                    req
    ...
    async renderRoute(route, domain, res, req)
    {
        ...
        return res.send(await this.renderWithTemplateContent(
            content,
    ...
    async renderWithTemplateContent(content, data, domain, req, route)
    {
        ...
        let siteHandle = this.resolveDomainToSiteKey(domain)
            + (routerKey ? ' '+routerKey : '')
    ...
    async renderNotFound(domain, res, req)
    {
        ...
        if(notFoundPath){
            ...
            if(content){
                res.status(404);
                return res.send(await this.renderWithTemplateContent({content}, {}, domain, req, null));
            }
        }
