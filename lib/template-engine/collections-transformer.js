    ...
    async processCollectionBase(
        ...
        if(paginationOptions){
            let mergedParams = this.paginationHandler.mergeCollectionParameters(
                this.parseCollectionTemplateParams(filtersJson, queryOptionsJson),
                this.paginationHandler.extractCollectionKeyFromRequest(
            ...
            collectionData = await this.loadEntityData(entity, mergedParams.filters, {
                limit: mergedParams.limit,
                offset: paginationData.offset,
                sortBy: mergedParams.sortBy,
                sortDirection: mergedParams.sortDirection
            }, relationsString, tableName);
            this.restoreEntityState(entity, originalState);
            finalContent = this.renderEngine.render(
                1 < paginationData.totalPages ? this.loadPaginationTemplate(paginationOptions.containerName, domain) : '{{&collectionContentForCurrentPage}}',
                Object.assign(
                    {},
                    paginationData,
                    {
                        collectionContentForCurrentPage: await this.renderCollectionLoop(
                            loopContent,
                            collectionData,
                            domain
                        )
        ...
        if(!paginationOptions){
            finalContent = await this.renderCollectionLoop(
                loopContent,
                await this.fetchCollectionForTemplate(
                    tableName,
                    filtersJson,
                    queryOptionsJson,
                    relationsString
